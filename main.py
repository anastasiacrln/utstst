...
fake_users_db = {
    "asdf": {
        ...
        "hashed_password": "$2b$12$0r9lgWZkFD4wavSgkvUHX.SCmjXAFuhdqEVccHcunyvtyGWBGKg5a",
        "disabled": False,
    }
}
...
@app.get('/menus')
async def show_menu(current_user: User = Depends(get_current_active_user)):
    return data['menu']
...
@app.post('/menu') 
async def add_menu(menu_item: Item, current_user: User = Depends(get_current_active_user)): 
    data['menu'].append({
        "id" : menu_item.id ,
        "name" : menu_item.name
    })
...
@app.put ("/menu/")
async def update_item(item : Item, current_user: User = Depends(get_current_active_user)):
    chosen_id = -1
    i = 0
    null = -1
